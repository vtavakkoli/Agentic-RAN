...
class AgenticPolicyEngine:
    ...
    def decide(self, observation: NetworkObservation, intent: SLAIntent | str | None = None) -> PolicyDecision:
        ...
        no_safe_candidate = not safe_candidates
        if not safe_candidates:
            # A neutral balanced action is still returned as an explicit non-executable
            # recommendation, but it must never be relabelled safe when hard critics fail.
            selected = next(candidate for candidate in evaluations if candidate.policy == "balanced")
            selection_pool = [selected]
        else:
            selection_pool = safe_candidates
            selected = max(
                selection_pool,
        ...
        uncertainty = self.uncertainty_estimator.assess(
            proposal_probs, observation, ood_score, selected.twin_uncertainty
        )
        confidence = self._confidence(selected, selection_pool, uncertainty.combined)
        ...
        trace = DecisionTrace(
            ...
            pareto_policies=self.pareto.front(safe_candidates) if safe_candidates else [],
    ...
    def _evaluate(self, observation, policy, proposal_probability, proposal_probabilities, ood_score, intent):
        twin = self.world_model.predict(observation, policy, horizon=self.planning_horizon)
        predicted = twin.final
        base_reasons = self._safety_reasons(observation, policy, predicted)
        temporal_reasons = self.temporal_guard.assess(observation.cell_id, policy.name)
        provisional_uncertainty = self.uncertainty_estimator.assess(
            proposal_probabilities, observation, ood_score, twin.uncertainty
        )
        if ood_score > 0.72 and policy.name not in {"balanced", "coverage_recovery"}:
            base_reasons.append("aggressive policy is blocked for a strongly out-of-distribution observation")
        critics = CriticSuite.evaluate(
            observation, predicted, policy.name, base_reasons, provisional_uncertainty, intent, temporal_reasons
        )
        score = self._utility_score(observation, predicted, intent)
        if CriticSuite.rejected(critics):
        ...
        return CandidateEvaluation(
            ...
            utility_score=score,
            safe=not CriticSuite.rejected(critics),
            safety_reasons=list(critics.reasons),
    ...
    def _safety_reasons(self, observation, policy, predicted):
        ...
        if (
            policy.name == "energy_saver"
            and str(observation.slice_type) == "URLLC"
            and (observation.latency_ms > 8.0 or observation.packet_loss_pct > 0.1)
        ):
    ...
    def _heuristic_candidate(observation):
        ...
        if str(observation.slice_type) == "URLLC" and (
            observation.latency_ms > 12.0 or observation.packet_loss_pct > 0.35
        ):
        ...
        if (
            str(observation.slice_type) == "eMBB"
            and observation.throughput_demand_mbps > observation.downlink_mbps * 1.15
        ):
    ...
    def _candidate_rationale(observation, policy, predicted, world_model):
        ...
        return (
            f"{policy.description} {world_model} predicts: latency {latency_delta:+.1f} ms, "
    ...
    @staticmethod
    def _critique(selected, rejected, safety_override, uncertainty_level, ood_score, no_safe_candidate=False):
        suffix = f" Uncertainty is {uncertainty_level}; OOD score is {ood_score:.2f}."
        if no_safe_candidate:
            return (
                "No candidate satisfied every hard constraint; balanced is returned only as a non-executable "
                "no-op recommendation." + suffix
            )
        if safety_override:
            return (
                "The highest-probability proposal was rejected by independent critics; "
                f"'{selected.policy}' is the best safe alternative." + suffix
            )
        if rejected:
            return (
                f"The critics rejected {', '.join(rejected)} and retained the highest-utility safe candidate." + suffix
            )
        return (
            "All evaluated candidates passed the configured critics; the highest combined utility and proposal "
            "confidence won." + suffix
        )
