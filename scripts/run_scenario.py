...
def run(
    ...
    results_dir = Path(results_dir) if results_dir is not None else Path("results") / "train" / "artifacts" / scenario_name
    ...
    scenario_use_agentic = use_agentic_policy
    ...
    multi_task = use_action_head and config.model_type.startswith("agentic_")
    ...
    if eval_trim > 0:
        ...
        action_pred = action_pred[eval_trim:]
        confidence = confidence[eval_trim:]
        test_raw_aligned = test_raw_aligned.iloc[eval_trim:].reset_index(drop=True)

    # Keep all arrays aligned. This is essential for the agentic-policy section:
    # evaluate_predictions only computes action metrics when action_true/action_pred
    # have exactly the same length.
    n_eval = min(len(y_eval), len(y_pred), len(test_actions), len(action_pred), len(confidence), len(test_raw_aligned))
    y_eval = y_eval[:n_eval]
    y_pred = y_pred[:n_eval]
    test_actions = test_actions[:n_eval]
    action_pred = action_pred[:n_eval]
    confidence = confidence[:n_eval]
    test_raw_aligned = test_raw_aligned.iloc[:n_eval].reset_index(drop=True)

    is_agentic_model = config.model_type.startswith("agentic_")
    ...
    model_metadata = {
        ...
        "has_action_head": bool(multi_task),
        "is_agentic_model": is_agentic_model,
        "agentic_policy_note": "Action labels are generated by the interpretable agentic policy and used for multi-task action-head supervision when the model is agentic.",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-dir", default=None, help="Optional output directory. Defaults to results/train/artifacts/<scenario>.")
