...
def _scenario_folder(results_root: Path, name: str) -> Path:
    artifact_folder = results_root / "train" / "artifacts" / name
...
def aggregate(results_root: Path = Path("results")) -> Path:
    ...
    for name, scenario in SCENARIOS.items():
        folder = _scenario_folder(results_root, name)
        ...
        row = {
            ...
            "use_agentic_policy": (metadata or {}).get("use_agentic_policy", False),
            "is_agentic_model": (metadata or {}).get("is_agentic_model", False),
            "avg_decision_confidence": agentic_summary.get("average_confidence", float("nan")),
            "dominant_agentic_action": max(agentic_summary.get("action_distribution", {"n/a": 0}).items(), key=lambda kv: kv[1])[0] if agentic_summary.get("action_distribution") else "n/a",
            "agentic_policy_labels_note": agentic_summary.get("labels_note", ""),
    ...
    model_family_comparison = leaderboard[
        ["scenario", "model_type", "sequence_length", "r2", "rmse", "mae", "smape", "wmape", "composite_score", "action_accuracy", "action_macro_f1"]
    ...
    for scenario_name in leaderboard["scenario"].tolist():
        pred_path = _scenario_folder(results_root, scenario_name) / "predictions.csv"
        if pred_path.exists():
            try:
    ...
    agentic_policy_cols = [
        ...
        "dominant_agentic_action",
    ]
    agentic_policy_df = df.loc[:, agentic_policy_cols].copy()
    agentic_policy_df = agentic_policy_df[
        (agentic_policy_df["use_agentic_policy"] == True)  # noqa: E712
        | (agentic_policy_df["is_agentic_model"] == True)  # noqa: E712
        | agentic_policy_df["action_accuracy"].notna()
    ]
    ...
    html = [
        ...
        "<div class='section'><h2>Executive summary</h2><p>This report benchmarks only integrated agentic-policy models with a learned action head for slice-aware RAN scheduling/resource actions across eMBB, MTC, and URLLC. Forecast-only models were removed from the main study scope.</p></div>",
        ...
        "</div>",
        "<div class='section'><h2>Agentic policy section</h2>",
        "<p>This section reports the interpretable agentic-policy labels and the learned action head. The policy maps RAN state to resource-allocation or scheduler-switch actions, then the agentic models learn a multi-task throughput/action objective.</p>",
        _to_html_table(agentic_policy_df) if not agentic_policy_df.empty else "<p>No agentic-policy outputs found. Make sure the agentic scenarios are enabled and the action head is active.</p>",
        "<p><em>Important:</em> action labels are pseudo-labels generated by the rule-based agentic policy, not operator ground truth. Use policy-consistency metrics together with safety-rule checks and slice-specific KPIs for control-quality claims.</p>",
        "</div>",
        "<div class='section'><h2>Agentic action-head comparison</h2>",
        _to_html_table(success_df[["scenario", "model_type", "r2", "rmse", "mae", "wmape", "action_accuracy", "action_macro_f1", "composite_score"]]),
        "</div>",
    ...
    if successful_scenarios:
        ...
        for scenario_name in successful_scenarios:
            folder = _scenario_folder(results_root, scenario_name)
            pred_abs = folder / "plots" / "predictions_vs_truth.png"
            try:
                pred_rel = pred_abs.relative_to(results_root).as_posix()
            except ValueError:
                pred_rel = pred_abs.as_posix()
    ...
    html.insert(
        -1,
        "<div class='section'><h2>Scientific wording note</h2><p>The action-decision metrics are pseudo-label based and should be described as policy-consistency metrics, not as real operator labels. Control quality should be argued using slice-specific KPIs, action-switch behavior, safe fallback rate, and safety-constraint behavior.</p></div>",
    )
